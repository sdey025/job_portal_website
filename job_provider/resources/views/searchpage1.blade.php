      ...
      <div class="col-md-5 mt-2">
        <form method="post" action="/searchpage1">
          ...
          <div class="form-group">
            <div class="row">
              <input type="email" class="form-control col-md-5 " name="jobseeker" id="exampleFormControlInput1" placeholder="Search jobseekers">
              ...
              @if (Route::has('login'))
                  <div class="top-right links">
                      @auth
                         ...
                         <a href="{{ url('/home') }}" class="dropdown-item text-dark "><center>See who applied<br> for Job</center></a>
...
@foreach($data as $row)
    <tr>
      <td><img src="http://localhost/jobfinder.com/User/public/public/storage/{{ $row->avatar }}" style="width:60px; height:65px;"></td>
      <td><a href="/profile" class="text-light" style="text-decoration:none">{{$row->name}}<br><small>From: {{$row->address}}</small></a></td>
      <td>{{$row->curr_job_post}}<br><small>At: {{$row->curr_job_com}}({{$row->curr_job_joining}})</small></td>
      <td>{{$row->ug_inst}}<br><small>Affiliated from: {{$row->ug_board}}</small></td>
      <td>{{$row->skill}}</td>
      <td><a href="/profile/{{$row->id}}" class="text-light" style="text-decoration:none">View Profile</a></td>
    </tr>
    @endforeach
  </table>
</div>
