              ...
              @if (Route::has('login'))
                  <div class="top-right links">
                      @auth
                         ...
                         <a href="{{ url('/network') }}" class="dropdown-item text-dark " active><center>Connections</center></a>
...
<div class="container">
  <div class="row">
    <div class="col-md-6 border border-primary border-right-4 border-left-0 border-bottom-0 border-top-0">
    <center><button type="button" class="btn btn-outline-success btn-lg" disabled>Followers &nbsp<span class="badge badge-success"><?php
           $flag=0;
            ...
            if (isset($result->num_rows) && $result->num_rows > 0) {
                            while($rows = $result->fetch_assoc()) {
                             if( $rows["reciever_id"] == Auth::user()->id){
                              $flag++;
                             }
                           }echo $flag;
                          }
    ...
    <center>
      ...
      <ul class="list-group list-group-flush">
        @if(Auth::user()->id == $row->reciever_id)
        <li class="list-group-item "> <?php
               $flag=0;
                ...
                if (isset($result->num_rows) && $result->num_rows > 0) {
                                while($rows = $result->fetch_assoc()) {
                                 if( $rows["id"]== $row->sender_id){
                                  echo "<img src='http://localhost/jobfinder.com/User/public/public/storage/".$rows["avatar"]."' class='rounded-circle' style='width:70px; height:70px;'>&nbsp".$rows["name"]."<br><small>".$rows["address"]."</small><a style='text-decoration:none' href='/message/".$rows["id"]."'>";
                                 }
                                }
                              }
                 ?> <button type="button"  class="btn btn-primary btn-sm"><input type="hidden" name="threads" value="threads">Message</button></a><form method='post' action='/network/{{$row->fid}}'> @csrf {{method_field('DELETE')}}  <input type="submit" value="Remove" class="btn btn-warning btn-sm float-right"></form></li>
        @endif
      </ul>
      @endforeach
    </center>
    </div>
    <div class="col-md-6 border border-primary border-right-0 border-left-4 border-bottom-0 border-top-0">
      <center><button type="button" class="btn btn-outline-dark btn-lg" disabled>Following &nbsp<span class="badge badge-dark"><?php
             $flag=0;
              $sql = "SELECT * FROM followsystem";
                $result = $conn->query($sql);
              ...
              if (isset($result->num_rows) && $result->num_rows > 0) {
                              while($rows = $result->fetch_assoc()) {
                               if( $rows["sender_id"] == Auth::user()->id){
                                $flag++;
                               }
                             }echo $flag;
                            }
        ...
        <ul class="list-group list-group-flush">
          ...
          <li class="list-group-item">    <?php
                 $flag=0;
                  ...
                  if (isset($result->num_rows) && $result->num_rows > 0) {
                                  while($rows = $result->fetch_assoc()) {
                                   if( $rows["id"]== $row->reciever_id){
                                    echo "<img src='http://localhost/jobfinder.com/User/public/public/storage/".$rows["avatar"]."' class='rounded-circle ' style='width:70px; height:70px;'>&nbsp".$rows["name"]."<br><small>".$rows["address"]."</small><a style='text-decoration:none;' href='/message/".$rows["id"]."'>";
                                   }
                                  }
                                }
          ?>    <button type="button"  class="btn btn-primary btn-sm">Message</button></a><form method="post" action="/network/{{$row->fid}}">@csrf {{method_field('DELETE')}}<input type="submit" value="Unfollow" class="btn btn-warning btn-sm float-right"></form></li>
          @endif
        </ul>
        @endforeach
      </center>
