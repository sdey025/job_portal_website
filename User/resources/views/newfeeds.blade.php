...
<!--/.Navbar -->
<br>
<center><span class="badge badge-danger">Latest feeds from connections</span></center>
<br>

@foreach($posts as $data)
@if($data->uid == $data->reciever_id && Auth::user()->id == $data->sender_id && $data->uid == $data->user_id)
<div class="container">

<div class="media" style="background-color:#F0F0F1;">
  <img src="http://localhost/jobfinder.com/User/public/public/storage/{{ $data->avatar }}" class="mr-3 ml-3 mt-3 rounded-circle border border-dark-3" style="width:100px; height:100px;" alt="...">
   <div class="media-body">
     <h5 class="mt-4">{{$data->name}}</h5><small>{{$data->p_created_at}}</small><br>
     <b>{{$data->post_content}}</b>
     <div>
       <span class="badge badge-success">Replies</span>
       <li class="list-group-item" style="width:850px; height:90px;">        <?php $yes=0;
              // $flag=0;
               ...
               if (isset($result->num_rows) && $result->num_rows > 0) {
                             //    echo "<table><tr><th>ID</th><th>Name</th></tr>";
                                 ...
                                 while($rows = $result->fetch_assoc()) {
                                if($data->uid == $rows["master_user_id"] && $data->pid == $rows["post_id"]){
                                  ...
                                  ?></li>
                      <br>
                      <i class="fa fa-reply text-dark ml-3"></i>


                        <form method="post" action="/newfeeds/{{$data->pid}}">
                          @csrf
                          <div class="row">
                          <textarea class="form-control" name="comment" style="width:800px;" id="exampleFormControlTextarea1" rows="3"></textarea>
                          <input type="hidden" name="mui" value="{{$data->uid}}">
                          <input type="submit" style="height:50px" class="btn btn-outline-success ml-3 mt-3">
                          </div>
                        </form>
<br>
