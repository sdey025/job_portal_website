...
<center><span class="badge badge-primary">Notifications</span></center>
<br>
...
<?php
       ...
       $flag=0;
        $sql = "SELECT followsystem.sender_id, followsystem.reciever_id, followsystem.created_at as f_crt, comments.comment, comments.commenter_name, comments.other_user_id, comments.created_at as c_crt, comments.master_user_id FROM followsystem JOIN comments";
          $result = $conn->query($sql);
        ...
        if (isset($result->num_rows) && $result->num_rows > 0) {
                      //    echo "<table><tr><th>ID</th><th>Name</th></tr>";
                          ...
                          while($rows = $result->fetch_assoc()) {
                         if(Auth::user()->id == $rows["reciever_id"] && $row->id == $rows["sender_id"]){
                              echo "<li class='list-group-item'>".$row->name." Started Following you.  at ".$rows["f_crt"]."</li>";
                            }
                        if(Auth::user()->id == $rows["master_user_id"] && $row->id == $rows["other_user_id"] && $row->name == $rows["commenter_name"] )
                          echo "<li class='list-group-item'>".$row->name." commented on your post at ".$rows["c_crt"]."</li>";
                          }
